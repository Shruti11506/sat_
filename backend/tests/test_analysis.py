...
def test_create_analysis_invalid_type(client):
    ...
    response = client.post(
        "/api/v1/analysis",
        json={"imagery_id": imagery_id, "analysis_type": "not_a_real_type", "query": "What is visible?"},
    )
...
def test_create_analysis_empty_query(client):
    imagery_id = _register_imagery(client)

    response = client.post(
        "/api/v1/analysis",
        json={"imagery_id": imagery_id, "analysis_type": "vqa", "query": ""},
    )
    assert response.status_code == 422
    body = response.json()
...
def test_create_analysis_whitespace_only_query(client):
    imagery_id = _register_imagery(client)
...
def test_create_analysis_response_shape(client):
    imagery_id = _register_imagery(client)
    ...
    assert data["imagery_id"] == imagery_id
    assert data["analysis_type"] == "vqa"
    assert data["query"] == "What type of land cover is visible in this image?"
...
def test_get_job(client):
    ...
    assert body["data"]["model_name"] is None
